...
class AddChild extends Component {
    ...
    else
    {
      ...
      // if(this.state.go!=true)
        return(      
        <form>
         <Administer> <button id="Userchild" onClick={this.p}>אשר הרשמה</button></Administer>
        </form>)
        }
      // else
       // return(<form>{alert('welcome'+this.state.ChildName)}
...
//onClick={() => this.newUserHandler(this.state)}onSubmit={() => this.newUserHandler(this.state.user)}<Link to={'Administer'}></Link> 
   ...
   // let f= await axios.post('User/AddUser',us).catch((err)=>alert(err)).then((res)=>alert(res.data))
         //alert('before')
      ///  let f=await axios.post('User/AddUser',us).catch((err)=>alert(err)) 
       // alert('f'+f)
    // let r;
        //if(f)
        //{
        //  alert('f.data'+f.data)
   // r=await axios.post('PropertyChildren/AddChildren',ch).catch((err)=>alert(err)).then('tgyhtgyh')
        //  alert('r.data'+r.data)
      // }
